...
class Predictor(object):
    ...
    def run(self):
        try:
            ...
            os.environ["IUPred_PATH"] = os.path.join(self.bin_directory,
                                                     "bin{}".format(self.architecture))
            output = subprocess.check_output(
                shlex.split(self.command), stderr=devnull, cwd=self.cwd, env=os.environ)
            ...
            logging.error('%s | %s crashed with error <%s>', acc, self.shared_name,
                          '; '.join(e.output.decode('utf-8').split('\n')))

            logging.debug('%s | %s crashed with error <%s>', acc, self.shared_name,
                          '; '.join(e.output.decode('utf-8').split('\n')), exc_info=True)
            ...
            logging.error('Unhandled exception encountered', stack_info=True)
...
class IUPredL(Predictor):
    ...
    def __init__(self, _input_file, _bin_directory, _architecture, _threshold):
        ...
        logging.debug('IUPred_PATH: %s', os.environ.get("IUPred_PATH"))

        bin_path = os.path.join(_bin_directory, "bin{}".format(_architecture))
        self.command = os.path.join(bin_path, 'iupred_string {} long'.format(self.input_seq))
...
class IUPredS(Predictor):
    ...
    def __init__(self, _input_file, _bin_directory, _architecture, _threshold):
        ...
        bin_path = os.path.join(_bin_directory, "bin{}".format(_architecture))
        self.command = os.path.join(bin_path, 'iupred_string {} short'.format(self.input_seq))
...
class DisEMBL(Predictor):
    ...
    def __init__(self, _input_file, _bin_directory, _architecture, _thresholds):
        ...
        self.command = 'python3 {0}/DisEMBL.python3.py 8 8 4 1.2 1.4 1.2 {0}/bin{2} {1}'.format(
            self.bin_directory, self.input_file, self.architecture)
...
class DynaMine(Predictor):
    ...
    def parse(self, output):
        ...
        if probs:
            return [Prediction(self.tag, probs, self.threshold, self.types, invert=True)]
